...
plt.plot(times_orig[ind_start:]/secinyear,vsh_of_t/vsh_t0)
plt.xlabel('Time (yr)')
plt.ylabel('vel')
plt.xscale('log')
plt.yscale('log')
...
rundir = args.save_dir

#*secinyear # 1e-7*secinyear #1e4*secinyear
t0 = t0_in #secinyear converted to dynamical time
print('times/tdyn_t0',times/tdyn_t0,t0)
dt0 =dt_scale_in/np.abs(c1(t0)) #should be dt_scale * initial dynamical time

final_age = tf_in #*secinyear #1.6e7*secinyear #1.6e9*secinyear #ages_array[-1]
...
# plt.plot(gamma_e_vals,df_dx2(f(np.zeros_like(gamma_e_vals),a_func(gamma_e_vals,t_i)),delta_gamma))
# plt.plot(gamma_e_vals,q_e_of_t(gamma_e_vals,t_i))
# plt.yscale('log')
# plt.xscale('log')
# plt.show()
...
while (t_i < final_age) and (count < max_step) and (dt_i > min_dt) and (stop == False):
    ...
    if count % print_interval == 0:
        ...
        print(f'y_next:{y_next[0]:1.2E},{y_next[-1]:1.2E}')
                # print('a_func',afunc(gamma_e_vals,t_i)[0],afunc(gamma_e_vals,t_i)[-3:])
        # print('A_func',Afunc(gamma_e_vals,t_i)[0],Afunc(gamma_e_vals,t_i)[-3:])
        print('y_i',y_i[0],y_i[-3:])
        # plt.plot(gamma_e_vals,f(y_i,a_func(gamma_e_vals,t_i)))
        # plt.plot(gamma_e_vals,y_i,zorder=3)
        # plt.plot(gamma_e_vals,y_next,zorder=3)
        # # plt.plot(gamma_e_vals,afunc(gamma_e_vals,t_i)*df_dx(y_i,delta_gamma))
        # # plt.plot(gamma_e_vals,-afunc(gamma_e_vals,t_i)*df_dx(y_i,delta_gamma))
        # # plt.plot(gamma_e_vals,-df_dx(y_i,delta_gamma))
        # # plt.plot(gamma_e_vals,-a_func(gamma_e_vals,t_i),ls='--')
        # # plt.plot(gamma_e_vals,q_e_of_t(gamma_e_vals,t_i),ls=':')
        # plt.xscale('log')
        # plt.yscale('log')
        # plt.show()
        # plt.close()
        # plt.figure()
        # plt.plot(gamma_e_vals,-afunc(gamma_e_vals,t=t_i))
        # plt.plot(gamma_e_vals,Afunc(gamma_e_vals,t=t_i))
        # plt.plot(gamma_e_vals,Qfunc(gamma_e_vals,t=t_i))
        # plt.yscale('log')
        # plt.xscale('log')
        # plt.show()
        # plt.close()
