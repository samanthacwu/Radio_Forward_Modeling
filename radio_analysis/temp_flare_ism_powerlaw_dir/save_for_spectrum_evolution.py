...
deltav_2 = shell_data[:,3] #cm/s
rho_const = shell_data[:,4] #g/cm^3
rhofl_2 = shell_data[:,5] #g/cm^3
int_rhofl_1_sq_dr = shell_data[:,6] #g^2/cm^5 
...
plt.plot(rsh/215/Rsun,rho_const,label='ISM constant density')
...
# plt.title(r'SN: $E=10^{51}$ erg, $M_{\rm ej}=1\, M_{\odot}$, Progenitor: $M_{\rm He}=2.898\,M_{\odot}$, $P_{\rm orb}=10$d')
plt.plot(times/secinyear,rho_const,label='ISM constant density')
...
if not args.Bfield:
    ...
    np.savez(save_dir+'/shock_data.npz',times=times,rsh_of_t=rsh,vsh_of_t=vsh,rho_const=rho_const,deltav2_of_t=deltav_2,rho2_of_t=rhofl_2,int_rhofl_1_sq_dr=int_rhofl_1_sq_dr) 
