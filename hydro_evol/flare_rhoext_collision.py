#example of own flare_ext_collision.py
...
def evolve_flares(M_flare,rho_ext_funcs,v_min_c,v_max_c,r_ins,r_outs,profile_times_arr,stop_ratio=11000,p=0.5,
	...
	for i, (rho_ext_func, r_in, r_out, profile_time) in enumerate(zip(rho_ext_funcs,r_ins,r_outs,profile_times_arr)):
		...
		rho_ext_init = rho_ext_func(rsh_0) 
		rho12_ratio = rho_flare_2_init/rho_ext_init
		vsh_0 = (v_max * math.sqrt(rho12_ratio)) / (1. + math.sqrt(rho12_ratio))
		#vsh_0 = v_max 
		print('Integrating to outer radius of r_out=',f'{r_out:1.2E}', 'cm for r_in =', f'{r_in:1.2E}', 'cm', 'vsh_0 =', vsh_0/c, 'c')
		...
		while t < stop_ratio*t0 and rsh < r_out:
			# print(rsh/r_out)
			...
			dMdt_arr = np.append(dMdt_arr, 4.*math.pi*rsh**2*(rho_fl_2*(v_fl_2-vsh) + rho_ext*vsh))
			Msh += 4.*math.pi*rsh**2*(rho_fl_2*(v_fl_2-vsh) + rho_ext*vsh) * dt
			rsh += vsh * dt
			vsh += (4.*math.pi*rsh_old**2/Msh) * (rho_fl_2*(v_fl_2-vsh)**2 - rho_ext*vsh**2) * dt
			t += dt
			# append
			...
			rho1_arr = np.append(rho1_arr, rho_ext)
		...
		print('stopped for time ratio', t/(stop_ratio*t0), 'rsh/r_out=', rsh/r_out)
		# plot evolution of shell parameters
		ax1.plot((t_arr-t0)/yr_to_sec, rsh_arr, ls=ls_arr[i], color=color_array[i], label=r't =%g d' % (profile_time))
		...
		save_dir = data_dir+f'shell_evolution_Mflare_{M_flare/Msun:1.0E}_profiletime_{profile_time:1.2E}/'
		if not os.path.exists(save_dir):
			os.mkdir(save_dir)
		else:
			print('Directory exists')
		# np.savetxt('shell_evolution_rhoISM_%ge-24_cgs.txt' % (rho_ext0/1e-24), np.c_[t_arr, rsh_arr, vsh_arr, v2_arr-vsh_arr, rho1_arr, rho2_arr, rho1sq_int_arr], header='Mfl=%gMsun, vmin=%gc, vmax=%gc, power-law-index=%g\ntime [s], rsh [cm], vsh [cm/s], Deltav_2 [cm/s], rho_ext [g/cm3], rhofl_2 [g/cm3], int_rhosq_1dr [g^2/cm^5]' % (M_flare/Msun, v_min/c, v_max/c, p), fmt='%.8g')
		...
		# ax0.set_xscale('log')
		# ax0.set_title(r'$\rho_{\rm ISM} =%g \times 10^{-24}$ g/cm$^3$' % (rho_ext0/1e-24))
		# ax0.grid(linestyle=':')
		...
		# ax0.legend()
		# fig0.tight_layout()
		# fig0.savefig('velocity_evolution_rhoISM_%ge-24_cgs.pdf' % (rho_ext0/1e-24))
	# finish
	ax1.legend()
	ax2.legend()
	ax3.legend()
